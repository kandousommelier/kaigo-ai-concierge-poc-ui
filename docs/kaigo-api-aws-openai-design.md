...
なお、`VITE_APP_KAIGO_API_ENDPOINT` を使って `kaigo_api` へ実際にHTTP/streaming接続するフロントエンドクライアントは、現段階では未実装である。本番有効化前に、Cognito ID tokenを付けた呼び出し、ストリーミング方式、エラー表示、タイムアウト、再試行方針を実装・検証する。
...
13. API Gateway生成の4XX/5XX応答にもCORSヘッダーが付いていること
14. Cognito認証失敗、期限切れ時にブラウザでCORS errorだけにならず、適切に扱えること
15. `VITE_APP_KAIGO_API_ENDPOINT` を使う実HTTP/streamingクライアントを追加・検証すること
16. レート制限、予算アラート、障害時表示を確認すること
17. 施設配布再開前に停止フラグ解除の承認手順を決めること
